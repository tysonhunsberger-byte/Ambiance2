"""Fixed Carla-backed VST host integration with proper Qt integration.

This version ensures proper Qt integration for showing plugin UIs.
"""
...
class QtApplicationManager:
    ...
    def __init__(self):
        ...
        if existing_app is None:
            ...
            # Create a timer to process Qt events periodically
            self._timer = QTimer()
    ...
    def is_available(self) -> bool:
        ...
        return HAS_PYQT5 and self._app is not None
...
class CarlaBackend:
    ...
    def __init__(self, base_dir: Path | None = None) -> None:
        ...
        if not self.root:
            self.warnings.append("Carla source tree not found – set CARLA_ROOT environment variable")
            return
        ...
        if HAS_PYQT5:
            try:
                ...
                if self._qt_manager and self._qt_manager.is_available():
                    self.warnings.append("Qt support enabled - plugin UIs available")
                else:
                    self.warnings.append("Qt initialization failed - plugin UIs unavailable")
            except Exception as exc:
                self.warnings.append(f"Qt initialization warning: {exc}")
        else:
            self.warnings.append("PyQt5 not installed - plugin UIs unavailable. Install with: pip install PyQt5")
    ...
    def _discover_root(self) -> Path | None:
        ...
        # Check system installations on Windows
    ...
    def status(self) -> dict[str, Any]:
        with self._lock:
            payload: dict[str, Any] = {
                ...
                "qt_available": HAS_PYQT5 and (self._qt_manager is not None and self._qt_manager.is_available()),
            }
            ...
            payload["capabilities"] = {
                "editor": bool(
                    ...
                    and HAS_PYQT5 
                    and self._qt_manager 
                    and self._qt_manager.is_available()
                ),
    ...
    def show_ui(self) -> dict[str, Any]:
        with self._lock:
            ...
            if not self._qt_manager or not self._qt_manager.is_available():
                raise CarlaHostError("Qt application not initialized - cannot show plugin UI")
            self._show_plugin_ui(True)
            return self.status()
    ...
    def _show_plugin_ui(self, visible: bool) -> None:
        ...
        if visible and not HAS_PYQT5:
            raise CarlaHostError("PyQt5 not installed - cannot show plugin UI")
        if visible and (not self._qt_manager or not self._qt_manager.is_available()):
            raise CarlaHostError("Qt not initialized - cannot show plugin UI")
        try:
...
__all__ = ["CarlaVSTHost", "CarlaHostError", "HAS_PYQT5"]
